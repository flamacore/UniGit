...
function Resolve-CodeSigningCertificateThumbprint {
  ...
  if ($env:UNIGIT_CERT_THUMBPRINT) {
    return $env:UNIGIT_CERT_THUMBPRINT.Trim()
  }
...
$requireSigning = $env:UNIGIT_REQUIRE_SIGNING -eq "1"
$subject = if ($env:UNIGIT_SIGN_SUBJECT) { $env:UNIGIT_SIGN_SUBJECT } else { "CN=UniGit Self Signed" }
$thumbprint = Resolve-CodeSigningCertificateThumbprint -Subject $subject

if (-not $thumbprint) {
  if ($requireSigning) {
    throw "No signing certificate was found. Generate one with 'npm run release:windows:cert' or set UNIGIT_CERT_THUMBPRINT."
  }
...
if ($env:UNIGIT_TIMESTAMP_URL) {
  $arguments += @(
    "/tr",
    $env:UNIGIT_TIMESTAMP_URL,
    "/td",
    "SHA256"
  )
}
